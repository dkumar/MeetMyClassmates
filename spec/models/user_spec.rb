...
describe User do
  ...
  it 'creates a public, non-recurring studygroup within a course user is enrolled in' do
    ...
    rtn_value = @owner.create_studygroup('studygroup_name', @course.title, false, Time.now, Time.now + 3600, 'soda', 10, false, false, [], [], nil)
  ...
  it 'creates a public studygroup within a course user is enrolled in and invites another user' do
    @owner.enroll_course(@course.title)
    @user.enroll_course(@course.title)
    rtn_value = @studygroup = @owner.create_studygroup('studygroup_name', @course.title, false, Time.now, Time.now + 3600, 'soda', 10, false, false, [], ['test@berkeley.edu'], nil)
    expect(rtn_value.instance_of?(Studygroup)).to eq(true)
    expect(Studygroup.exists?(rtn_value)).to eq(true)
  end
  ...
  it 'deletes studygroup user is owner of' do
    ...
    rtn_value = @owner.create_studygroup('studygroup_name', @course.title, false, Time.now, Time.now + 3600, 'soda', 10,
                                         false, false, [], [], nil)
    expect(rtn_value.instance_of?(Studygroup)).to eq(true)
